...
class Item(BaseModel):
    timestamp: str # 字符串会自动解析为datetime对象
...
def batch_predict(request: BatchRequest):
    ...
    if not abnormal_results:
        ...
        '''with open("E:/azhonggonghulian/tongfangshuiwu/yolov5-master/yolov5-master/output.md", "w", encoding="utf-8") as f:
            f.write("## 输出\n\n")
            for idx, item in enumerate(abnormal_results, start=1):
                f.write(f"{idx}. {item}\n")  # 有序列表格式'''
        return {
